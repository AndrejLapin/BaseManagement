...
public class Player : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        myBackpack = GetComponent<Backpack>();
    ...
    void UseItem()
    {
        if(Input.GetButtonDown(InputName.USE_ITEM) && currentGadget)
        {
            currentGadget.Use();
        }
        else if (Input.GetButtonUp(InputName.USE_ITEM) && currentGadget)
        {
            currentGadget.StopUsing();
        }
    }
    ...
    {
        return characterBody.transform.rotation;
    }

    public int AbsorbResource(ResourceType type, int amount)
    {
        return myBackpack.AddToBackpack(type, amount);
    }
}
